...
const album_fields =
  "id,media_type,media_url,thumbnail_url,username,timestamp,permalink";
  ...
  async downloadAlbum(parent, token) {
    ...
    album.data.forEach((element) => {
      if (
        ...
        (element.media_type === "VIDEO" && settings.instagram_allow_videos)
      ) {
        media.push({
          mediaId: parent.id,
          id: element.id,
          ...
          thumbnailUrl:
            element.media_type === "VIDEO" ? element.thumbnail_url : null,
        });
      }
    });
    return media;
